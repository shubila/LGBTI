...
system.time(
logit_partial_featimp <- FeatureImp$new(predictor_logit, loss = "ce")
)

system.time(
rf_partial_featimp    <- FeatureImp$new(predictor_rf, loss = "ce")
)


# save the feat imp objects of both models 

# save(logit_partial_featimp,file = "./output/ml_object/caret/partial/logit_partial_featimp.RData")
# save(rf_partial_featimp,file = "./output/ml_object/caret/partial/rf_partial_featimp.RData")
