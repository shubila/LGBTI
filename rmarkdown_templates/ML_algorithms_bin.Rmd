...
inTrain <- createDataPartition(lgbti_ML$open_at_work_bin, 
                               p = .7, 
                               list = FALSE, 
                               times = 1)
...
# system.time(
# m_logit <- train(open_at_work_bin ~ .,
#                       data = lgbti_train,
...
# set grid for hyper parameter mtry and min.node.size 
 ...
 #                                      round(sqrt((ncol(lgbti_ML)-1))),
 #                                      round((ncol(lgbti_ML)-1)/3),
 #                                        65,75,85),
 #                              splitrule = "hellinger",
 #                              min.node.size = 5
 # )
  ...
  rf_grid <- expand.grid(mtry = seq.int(5, 35, by=5),
                              splitrule = "hellinger",
                              min.node.size = 5
 )
  ...
  system.time(
    m_rf <-
      train(
        x = lgbti_train[!names(lgbti_train) %in% c("open_at_work_bin")],
        y = lgbti_train$open_at_work_bin,
        #data = lgbti_train,
        method = "ranger",
        trControl = ctrl,
        tuneGrid = rf_grid,
...
#m_xgb 
#varImp(m_rf)
# col_index <- varImp(m_rf )$importance %>% 
...
roc_logit <- roc(lgbti_test$open_at_work_bin, p_logit$Open) 

roc_cart <- roc(lgbti_test$open_at_work_bin, p_cart$Open) 

roc_rf  <- roc(lgbti_test$open_at_work_bin, p_rf$Open) 
...
ggroc(list(
  ...
  color = "grey",
  linetype = "dashed") +
  labs(subtitle = "ROC-AUC for the four classification models")
...
confusionMatrix(c_logit, lgbti_test$open_at_work_bin, mode = "everything", positive = "Open")
confusionMatrix(c_cart, lgbti_test$open_at_work_bin, mode = "everything", positive = "Open")
confusionMatrix(c_rf, lgbti_test$open_at_work_bin, mode = "everything", positive = "Open")
...
lgbti_test$Open_cart  <- p_cart$Open
lgbti_test$Open_rf    <- p_rf$Open
#lgbti_test$Open_xgb  <- p_xgb$Open

ggplot(lgbti_test, aes(x = Open_logit)) + 
  geom_histogram(binwidth = .05) + 
  facet_wrap(~ open_at_work_bin) + 
  xlab("Probability of Open for logit")

ggplot(lgbti_test, aes(x = Open_cart)) + 
  geom_histogram(binwidth = .05) + 
  facet_wrap(~ open_at_work_bin) + 
  xlab("Probability of Open for cart tree")

ggplot(lgbti_test, aes(x = Open_rf)) + 
  geom_histogram(binwidth = .05) + 
  facet_wrap(~ open_at_work_bin) + 
  xlab("Probability of Open for Random forest")
...
vip(m_rf, width = 0.5, aesthetics = list(fill = "green3"))

varImpPlot(m_rf,type=2)
varimp_rf <- varImp(m_rf)

roc_imp <- filterVarImp(x = lgbti_train[, -ncol(lgbti_train)], y = lgbti_train$open_at_work_bin)
head(roc_imp)
plot(varimp_rf,yaxt="n")
axis(2,cex.axis=0.5)
options(scipen = 999)
col_index <- varImp(m_rf)$importance %>% 
  mutate(names=row.names(.)) %>%
